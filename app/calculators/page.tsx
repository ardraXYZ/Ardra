...
const calculators = [
    ...
    },
    {
        id: "aster",
        title: "Aster",
        description: "Aster quest, rebates and ladder simulator (coming soon).",
        live: false
    },
    {
        id: "pacifica",
            ...
            <main className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-24 pt-28 sm:px-6 md:px-10">
                ...
                <section className="grid gap-6 md:grid-cols-3">
                    {calculators.map(calc => {
                        const body = (
                            <article className="group relative h-full overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_18px_80px_rgba(56,189,248,0.12)] transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-white/[0.07]">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 className="text-lg font-semibold text-white">{calc.title}</h2>
                                ...
                                <p className="mt-5 text-sm text-white/65">{calc.description}</p>
                                {calc.live ? (
                                    <Button asChild className="mt-8 rounded-full bg-cyan-500 px-6 text-black hover:bg-cyan-400">
                                        <Link href={calc.href!}>Open calculator</Link>
                                    </Button>
                                ) : (
                                    <Button disabled variant="outline" className="mt-8 rounded-full border-white/15 text-white/50">
                                        In development
                                    </Button>
                                )}
                            </article>
                        ...
                        return calc.live ? (
                            <Link key={calc.id} href={calc.href!} className="block h-full">
                                {body}
                            </Link>
                        ) : (
                            <div key={calc.id}>{body}</div>
                        )
                    })}
                </section>
            </main>
